...
def angle_vector_from_quat(quat): # TODO: not sure this is right
  w, normal = quat[0], quat[1:]
  angle = 2*acos(w)
  vector = 2*asin(w)*np.array(normal)
  return angle, vector
...
def base_values_from_trans(trans, axis=[0, 0, 1]):
  return RaveGetAffineDOFValuesFromTransform(trans, DOFAffine.X|DOFAffine.Y|DOFAffine.RotationAxis, axis)

def xyzt_from_trans(trans, axis=[0, 0, 1]):
  return RaveGetAffineDOFValuesFromTransform(trans, DOFAffine.X|DOFAffine.Y|DOFAffine.Z|DOFAffine.RotationAxis, axis)

def base_values_from_pose(pose):
  return base_values_from_trans(trans_from_pose(pose))

def is_upright(trans):
  return abs(abs(trans[2, 2]) - 1) < 1e-6 # NOTE - abs allows it to be on bottom or top face
