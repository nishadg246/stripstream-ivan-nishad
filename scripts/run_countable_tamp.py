...
def solve_countable_tamp(planner, search, visualize, display, verbose=False):
  ...
  if search == DEFAULT:
    ...
    search_fn = get_fast_downward('eager', verbose=verbose_search) # 'dijkstra | astar | wastar1 | wastar2 | wastar3 | eager | lazy
  ...
  if planner == INCREMENTAL:
    ...
    plan, universe = simple_focused(stream_problem, search=search_fn, greedy=True, dfs=True, verbose=verbose)
...
def main():
  ...
  solve_countable_tamp(planner, args.search.lower(), args.viewer, args.display)
