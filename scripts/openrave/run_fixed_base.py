...
def solve_tamp(env):
  ...
  cond_streams = [
    ...
    EasyListGenStream(inputs=[Q, Q2], outputs=[T], conditions=[],
                      ...
                      generator=sample_free_motion_fn(manipulator, base_manip, all_bodies), order=1, max_level=0),
    EasyListGenStream(inputs=[Q, Q2, G], outputs=[T], conditions=[],
                      effects=[HoldingMotion(Q, Q2, G, T)],
                      generator=sample_holding_motion_fn(manipulator, base_manip, body1, all_bodies), order=1, max_level=0),
    ...
    EasyTestStream(inputs=[T, P], conditions=[], effects=[CFreeTraj(T, P)],
                test=cfree_traj_fn(env, manipulator, body1, body2, all_bodies)),
  ]
