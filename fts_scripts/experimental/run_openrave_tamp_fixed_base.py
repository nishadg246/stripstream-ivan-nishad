...
def get_problem(env, problem, robot, manipulator, base_manip, bodies, initial_conf):
  ...
  transition = [
    Clause([Stable(X[O_P, O]), Grasp(nX[O_P, O]), # NOTE - not necessary but results in slowdown...
  ...
  sample_grasp_traj = sample_grasp_traj_fn(env, robot, manipulator, body1, all_bodies)
  sample_free_motion = sample_free_motion_fn(manipulator, base_manip, all_bodies)
  sample_holding_motion = sample_holding_motion_fn(robot, manipulator, base_manip, body1, all_bodies)
  ...
  samplers = [
    ...
    # Move trajectory
    Sampler([FreeMotion(Q, Q2, T)], gen=sample_free_motion, inputs=[Q, Q2], order=1, max_level=0),
    Sampler([HoldingMotion(Q, Q2, G, T)], gen=sample_holding_motion, inputs=[Q, Q2, G], domain=[Grasp(G)], order=1, max_level=0),
  ]
