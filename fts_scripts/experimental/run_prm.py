...
def create_problem(goal, obstacles=(), distance=.25, digits=3):
  ...
  #AreNearby = ConType([CONF, CONF], test=lambda q1, q2: get_distance(q1, q2) <= distance)
  #IsEdge = ConType([CONF, CONF], domain=[AreNearby([CONF, CONF])],
  #                 test=lambda q1, q2: is_collision_free((q1, q2), obstacles)) # CFree
