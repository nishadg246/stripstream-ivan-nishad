...
from stripstream.pddl.logic.predicates import EasyPredicate, EasyFunction
...
#  return (atom.predicate.name,  map(get_value, atom.args))
#  return atom.instantiate(dict(zip(atom.args, map(get_value, atom.args))))
    ...
    for arg in atom.args:
      if isinstance(arg, Parameter):
        if arg.name in param_names:
          ...
          param_names[arg.name] = arg # TODO - extend to several
  return param_names
...
def rename_easy(assignments):
  ...
  for name, value in assignments.iteritems():
    if type(value) in (EasyType, EasyPredicate, EasyFunction):
      value.name = name.lower()
    elif type(value) == EasyParameter:
      Parameter.__init__(value, name, value.type) # Mutates value
