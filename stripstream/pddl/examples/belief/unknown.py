...
def observable_problem(belief, goal):
  ...
  for obj in belief.objLoc.keys():
    ...
    for loc in locations:
      p = dist.prob(loc)
      cost = int(COST_SCALE*1./max(p, MIN_P)) # TODO - set to infinity if too large (equivalent to blocking)
  ...
  for obj in belief.objState.keys():
    ...
    for state in states:
      p = dist.prob(state)
      cost = int(COST_SCALE*1./max(p, MIN_P))
      initial_atoms.append(Initialize(InspectStateCost(OBJ(obj), STATE(state)), cost))
      #if p < MIN_CONFIDENCE:
      #  initial_atoms.append(NotAtLoc(obj, loc))

  for loc in belief.occupancies.keys():
    dist = belief.locOccDist(loc)
    p = dist.prob(False) # Not occupied
    if loc not in occupied:
      if p >= CLEAR_CONFIDENCE:
        initial_atoms.append(Clear(loc))
      #elif not free and p >= CLEAR_CONFIDENCE:
      #  pass
      else:
        initial_atoms.append(UnsureClear(loc))
    cost = int(COST_SCALE*1./max(p, MIN_P))
    initial_atoms.append(Initialize(InspectLocCost(LOC(loc)), cost))
    if loc == 'washer':
