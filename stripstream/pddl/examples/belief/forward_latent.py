...
class InferAtCostFn(CondStream):
  def __init__(self, prior):
    self.prior = prior
    super(InferAtCostFn, self).__init__([O, B], [], [], [], eager=True)
  class StreamFn(Stream):
    def get_values(self, **kwargs):
      self.enumerated = True
...
actions = [
  Action(name='transport', parameters=[O, L, L2, B, B2], # NOTE - Leslie and Tomas call this Move
    ...
    effect=And(BAt(O, B2), At(O, L2), Not(At(O, L)), Not(BAt(O, B)), #)),
               Cost(MoveCost(L, B)))),
  ...
  Action(name='infer_at', parameters=[O, L],
    condition=UnknownAt(O),
    effect=And(At(O, L), Not(UnknownAt(O)), #)),
               Cost(LookCost(B, L)))),
...
def forward_latent_policy(operators, goal): # TODO - avoid translating/save some work
  def fn(belief):
