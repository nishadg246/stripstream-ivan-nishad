...
from stripstream.pddl.examples.continuous_tamp.continuous_tamp_utils import are_colliding, in_region, \
  sample_region_pose, inverse_kinematics
...
#LegalPose = Pred(BLOCK, POSE] # TODO - verify that the pose is within the interval
...
def compile_problem(tamp_problem):
  ...
  actions = [
    ...
    #Action(name='press', parameters=[Q1, Q2],
    #  condition=AtConf(Q1),
    #  effect=And(AtConf(Q2), Not(AtConf(Q1)))),

    # TODO - make an associated button with each of these

    # NOTE - this is like the top grasp problems
    # TODO - forklift in order to add some pick constraints
    # Stacking

    # TODO - constraints and dynamic obstacles

    # TODO - do a true 2d version

    # TODO - side grasps and top grasps in 1D

    Action(name='clean', parameters=[B1, R], # Could quantify over the sink
      condition=And(InRegion(B1, R), IsSink(R)),
      effect=And(Cleaned(B1))),
    Action(name='cook', parameters=[B1, R], # Could quantify over the stove
      condition=And(InRegion(B1, R), IsStove(R)),
      effect=And(Cooked(B1))),
  ]

  axioms = [
    Axiom(effect=InRegion(B1, R), condition=Exists([P1], And(AtPose(B1, P1), Contained(B1, P1, R)))),
