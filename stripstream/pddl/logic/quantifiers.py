...
def combine_quantifiers(formula, Class): # TODO - what about simplifying with something in the way
...
class Quantifier(Formula):
  ...
  def __init__(self, args, formula):
    ...
    if not all(isinstance(arg, Parameter) for arg in args):
      raise ValueError('All quantifier arguments must extend class Parameter: %s'%args)
    if not isinstance(formula, Formula):
      raise ValueError('Quantifier formula must extend class Formula: %s'%formula)
    self.args = args
  ...
  def simplify(self):
    ...
    if not args: return formula.simplify()
  ...
  def substitute(self, atom, subformula):
    if isinstance(self.formula, stripstream.pddl.logic.atoms.Atom) and self.formula.predicate == atom.predicate:
      self.formula = subformula.instantiate(dict(zip(atom.args, self.formula.args)))
    else:
  ...
  #  return self._Complement(self.args, self.formula.de_morgan(sign=sign))
  def pddl(self):
    return '(%s (%s) %s)'%(self._pddl_name, ' '.join(arg.typed_pddl() for arg in self.args), self.formula.pddl())
  __repr__ = pddl

def has_existential_child(formula): # TODO - clean this up
...
class ForAll(Quantifier, Condition, Effect):
  ...
  def dequantify(self, constants):
    if has_existential_child(self.formula):
      raise NotImplementedError('Currently do not support ForAll([...], Exists(...))')
  ...
  def positive_supporters(self, atoms, constants):
    ...
    for formula in self.expand(constants):
      child_literals = formula.positive_supporters(atoms, constants) # TODO - just use the objects within atoms?
