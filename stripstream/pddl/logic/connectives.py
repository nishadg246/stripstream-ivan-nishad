...
class Connective(Formula):
  ...
  def __init__(self, *formulas):
    ...
    if not all(isinstance(f, Formula) for f in formulas):
      raise ValueError('All Connective formulas must extend class Formula: %s'%formulas)
    if not formulas: # TODO - handle case when there are zero atoms?
      raise ValueError('Must specify at least one Connective formula')
    self.formulas = formulas
  def get_atoms(self): return reduce(operator.or_, (f.get_atoms() for f in self.formulas))
  def get_formulas(self): return self.formulas
  def get_quantified(self): return reduce(operator.or_, (f.get_quantified() for f in self.formulas))
  def simplify(self):
    formulas = [f.simplify() for f in combine_connectives(self, self.__class__)]
  ...
  def substitute(self, atom, subformula):
    ...
    for i, formula in enumerate(self.formulas):
      if isinstance(formula, stripstream.pddl.logic.atoms.Atom) and formula.predicate == atom.predicate:
        new_formulas[i] = subformula.instantiate(dict(zip(atom.args, self.formulas[i].args)))
      else:
...
# TODO - just return the formula if only a single argument
...
class And(Connective, Condition, Effect):
  ...
  def get_literals(self):
    ...
    for combo in product(*dnfs):
      #yield tuple(reduce(operator.add, map(operator.itemgetter(i), combo)) for i in range(2))
  ...
  def positive_supporters(self, atoms, constants):
    ...
    for formula in self.formulas:
      ...
      if child_literals is None: return None
  ...
  def negative_supporters(self, atoms, constants):
    for formula in self.formulas:
      ...
      if child_literals is not None: return child_literals
    return None
...
class Not(Connective, Condition, Effect):
  ...
  def de_morgan(self, sign=True): return self.formula.de_morgan(sign=not sign)
  ...
  def positive_supporters(self, atoms, constants): # TODO - apply not to these?
  ...
  def __eq__(self, other): # NOTE - kind of strange to do this for just Not
...
class When(Connective, Effect):
  ...
  def __init__(self, formula1, formula2):
    ...
    """
    #assert formula1.is_condition() and formula2.is_effect()
  ...
  def delete(self, atoms, constants):
    ...
    return []
  #def replace(self, parameter_map):
  #  pass