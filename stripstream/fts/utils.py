...
def get_value(x):
  if type(x) != Constant or not isinstance(x.type, VariableType): return x
  return x.type.get_value(x)

def convert_plan(plan):
  if plan is None: return plan
...
def rename_variables(assignments):
  for name, value in assignments.iteritems():
    if type(value) == VarType:
      ...
      FreeParameter.__init__(value, name, value.type) # Mutates value
