...
class ConstraintType(object):
  ...
  def __init__(self, types=(), test=None, domain=(), satisfying=(), name=None, **kwargs):
    ...
    self.domain = tuple(domain) # TODO - how do I specify the domain constraints if I don't have variables?
...
Eq = ConType([OBJECT, OBJECT], test=lambda a, b: a == b, name='Eq') # TODO - should I allow ternary Eq?

# TODO - what if a sampler doesn't achieve anything? Just include Identity?
...
# TODO - STRIPStream doesn't correctly handle OBJECT types yet
DUMMY_CONSTRAINTS = (
  Identity,
  Unconstrained,
  Universe,
...
def convert_constraint(constraint):
  #assert not any(isinstance(item, VarMember) for item in self.values)
...
def make_test_stream(constraint_form):
  ...
  assert not constraint_form.domain # TODO - later support this
  params = [FreeParameter('%s'%i, dtype) for i, dtype in enumerate(constraint_form.types)]
  constraint = constraint_form(*params)
  test = lambda inputs: constraint_form.test(*[i.type.get_value(i) for i in inputs])
