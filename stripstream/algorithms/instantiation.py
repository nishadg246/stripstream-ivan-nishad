...
# TODO - what if overlap by two values?
# TODO - what if triangle of overlap? Should I just make it a DAG?
# TODO - could represent as a tree
# TODO - use when calling streams during the relaxed planning
...
def parameter_product(conditions, condition_values, objects, object_values):
  ...
  for ci, cond in enumerate(conditions):
    ...
    new_indices = [i for i, a in enumerate(cond.args) if a not in intersect]
    new_combos = []
    for combo in combos:
      for value in condition_values[ci]:
        if all(combo[params[a]] == value.args[cond_params[a]] for a in intersect):
          new_combos.append(combo + [value.args[i] for i in new_indices])
  ...
  if not params: # TODO - this should probably just yield the empty list?
    return
  param_order, _ = zip(*sorted(params.items(), key=lambda (_, i): i))
  values = [object_values[i] for i in new_indices]
  for value in product(*values): # TODO - could instead do the look again
    for combo in combos:
      yield dict(zip(param_order, combo + list(value)))

# NOTE - this allows specification of the replacement values (should merge with previous one)

# I guess I should just match up any effect that has the right value
...
def parameter_product2(parameters, parameter_values, conditions, condition_values):
  ...
  for cond in conditions:
    ...
    new_indices = [i for i, a in enumerate(cond.args) if a in free_parameters]
    for i in new_indices:
      combo_index[cond.args[i]] = len(combo_index)
    new_combos = []
    for combo in combos:
      for value in condition_values:
        if cond.predicate == value.predicate and \
            all(a1 in cond_parameters or a1 == a2 for a1, a2 in zip(cond.args, value.args)) and \
            all(combo[combo_index[a]] == value.args[cond_index[a]] for a in intersect_parameters):
          new_combos.append(combo + [value.args[i] for i in new_indices])
    combos = new_combos

  new_indices = [i for i, obj in enumerate(parameters) if obj not in combo_index]
  for i in new_indices:
    combo_index[parameters[i]] = len(combo_index)
  if not combo_index:
    return
  param_order, _ = zip(*sorted(combo_index.items(), key=lambda (_, i): i))
  values = [filter(lambda v: parameters[i].type == v.type, parameter_values) for i in new_indices]
  for value in product(*values): # TODO - could instead do the look again
    for combo in combos:
      yield dict(zip(param_order, combo + list(value)))
...
# TODO - the order in which is done might affect the performance...
# TODO - exploit understanding the intersection between conditions

def smart_instantiate_operator(operator, static_map, type_to_objects):
  arg_names = set() # TODO - make these tuples?
  arg_values = {frozenset()}

  # TODO - quantifiers, could have static predicates in quantifiers (this is at least true for axioms)
  instances = set()
  #for literals in operator.condition.get_literals(): # TODO - re-add this back in
  for literals in [operator.condition.get_atoms()]:
    for literal in literals:
      #if isinstance(literal, Not):
      #  if literal.formula.predicate in universe.stream_predicates:
      #    raise ValueError('Stream atom %s mentioned negatively in precondition of %s'%(literal.formula, operator))
      #if literal.predicate in universe.stream_predicates:
      if literal.predicate in static_map:
        ...
        for static_atom in static_map[literal.predicate]:
          if all(arg1.type == arg2.type for arg1, arg2 in zip(literal.args, static_atom.args)): # NOTE - run into trouble with the OBJECT type
            name_map = dict(zip(literal.args, static_atom.args))
            inter_values = frozenset((name, name_map[name]) for name in inter_names)
            inter_map[inter_values].append(frozenset(name_map.iteritems()))
    ...
    remaining_values = [type_to_objects.get(param.type, []) for param in remaining_names]
    ...
    for arg_value in arg_values:
      for combo in remaining_combos:
        ...
        instance = operator.instantiate([combined[param] for param in operator.parameters])
...
def smart_instantiate_operators(operators, universe):
  ...
  for atom in universe.get_initial_atoms():
    if not isinstance(atom, Initialize) and atom.predicate in static_map: # TODO - include static_predicates?
